...
class Camera
{
    ...
    public function __construct(array $cam)
    {
        $this->_origine = $cam['origin'];
        $this->_tT = new Matrix(array('preset' => Matrix::TRANSLATION, 'vtc' => $this->_origine->opposite()));
        $this->_tR = $this->_transpose($cam['orientation']);
        $this->_width = (float) $cam['width'] / 2;
        $this->_height = (float) $cam['height'] / 2;
